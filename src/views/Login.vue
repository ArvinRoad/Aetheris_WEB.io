...
<script setup>
    ...
    // 同步导入登录接口（避免async setup导致Suspense警告）
    // 异常处理：接口模块不存在时使用空实现，防止组件崩溃
    let loginApi = () => Promise.reject({ msg: '登录接口模块未配置' });
    try {
        const authModule = require('@/api/auth');
        loginApi = authModule.loginApi || loginApi;
    } catch (e) {
        console.warn('登录接口模块加载失败，使用默认空实现：', e);
    }
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            if (err.message?.includes('ERR_CONNECTION_REFUSED')) {
                ...
                errorMsg.value = `登录失败：${err.message || '未知错误'}`;
            }
        } finally {
